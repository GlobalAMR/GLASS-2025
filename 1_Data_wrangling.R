# #################################################################################################
# GLASS 2025 REPORT – DATA WRANGLING
# #################################################################################################
#
...
# Purpose:
# Prepare and clean all country, population, HAQI and AMR source data for the
# GLASS 2025 analyses. Produce linked, analysis-ready datasets at country- and
# age–sex–origin strata suitable for downstream standardisation and modelling.
#
# Key actions performed by this script:
# 1. Environment and inputs
#    - Load required R packages and helper functions.
#    - Define directories (raw, cleaned, output) and set analysis year (variable `year`).
#
# 2. Country metadata
#    - Read country master file, normalise names (e.g. "Côte d'Ivoire") and sort.
#    - Flag countries that reported AST in the selected `year` (AMR_GLASS_AST_2023).
#    - Parse and extract AMR/AMC enrollment dates and years.
#    - Export cleaned country file.
#
# 3. Population (age × sex)
#    - Read UN WPP 2024 mid-year population files (female & male).
#    - Convert per-1000 estimates to absolute counts (×1000), pivot from wide to long.
#    - Aggregate French overseas territories into FRA as required.
#    - Re-bin 5-year groups to 10-year bands (AgeCat10) and produce `pdataDM`.
#    - Export final population-by-age-sex dataset.
#    - NOTE: Source is UN WPP 2024 (mid-year estimates); PopSourceComments = "WPP2024July".
#
# 4. HAQI (IHME)
#    - Read IHME HAQ data, normalise country names to match codelist.
#    - Extract HAQ index for 2019 and link to country metadata.
#    - Export HAQI-country linked dataset.
#
# 5. AMR – country-level (adataAC)
#    - Read country AMR file and create `combined` keys (Specimen–Pathogen–Antibiotic).
#    - Clean trailing whitespace in AntibioticName.
#    - Create grouped antibiotic classes where required:
#        • Third-generation cephalosporins (CTX/CAZ/CRO → 3GC)
#        • Methicillin resistance (OXA/FOX → MRSA)
#        • Co-trimoxazole (J01EE/SXT)
#    - For overlapping representations (single antibiotic vs class in 2017–2018),remove redundant records (keep the record with the max percent resistance).
#    - Compute PercentResistant and produce one best row per (Specimen, Pathogen, Class, Iso3, Year).
#    - Re-attach single-antibiotic CTX/CAZ/CRO rows for Escherichia coli BLOOD where required.
#    - Join population and HAQI metadata and export:
#        • EI_AMRdtaAC_Pop_country_HAQI_140325_EV.csv
#        • EI_AMRdtaAC_ANALYSES.csv (country-level analysis file)
#
# 6. AMR – age/sex/origin stratified (adataNT / adataNT_c)
#    - Normalise DemographicsOrigin labels and parse Sex, AgeCat10, and Origin.
#    - Aggregate raw rows to produce summed counts per (Iso3, Year, DemographicsOrigin, PathogenName, Antibiotic).
#    - Apply same antibiotic-class grouping logic as for country-level data.
#    - For each (Specimen, Pathogen, AntibioticClass, Iso3, Year, AgeCat10, Sex, DemographicsOrigin)
#      keep the record with maximal PercentResistant (slice_max without ties).
#    - Re-insert single CTX/CAZ/CRO rows for blood E. coli where applicable.
#    - Link to country metadata and population strata (pdataDM).
#    - Export EI_AMRdtaINT_Pop_Country_140325_EV.csv and EI_AMRdtaINT_ANALYSES_noTESTING.csv;  NOTESTING here means that the standardised 
#      testing rates are not yet added, as these are calculated in "2_Age_sex_standardised_testingrates.R"
#
# 7. Analysis dataset preparation
#    - Construct analysis-ready `adataAS` that:
#        • Keeps records with valid denominators or specified origins.
#        • Computes `amr_rate = Resistant / InterpretableAST`.
#        • Computes testing intensity proxy `BCI_1000000pop = InterpretableAST / TotalPopulation * 1e6`.
#        • Converts AgeCat10 into a numeric spline-friendly covariate `s_AgeCat10`.
#    - Produce trimmed datasets for modelling (filtering InterpretableAST thresholds >10 per country per year).
#
# 8. Exported outputs (examples)
#    - EI_Countrydta_AST_140325_EV.csv
#    - EI_PopdtaDM_140325_EV.csv
#    - EI_HAQIdta_Country_140325_EV.csv
#    - EI_AMRdtaAC_Pop_country_HAQI_140325_EV.csv
#    - EI_AMRdtaAC_ANALYSES.csv
#    - EI_AMRdtaINT_Pop_Country_140325_EV.csv
#    - EI_AMRdtaINT_ANALYSES_noTESTING.csv
#
# #################################################################################################
...
pacman::p_load(readxl, writexl, lubridate, zoo, ggplot2, tidyverse, Hmisc, stringr,lme4,reshape2, RColorBrewer,
               ...
               gridExtra, purrr, brms, cowplot, here, ggtext)
...
pdata = read.csv(paste0(dirDataRaw, "/EI_Popdta_110325_EV.csv"), sep=",")       
pdataF_raw = readxl::read_xlsx(paste0(dirDataRaw, "/UN_pop_raw/EI_Popdta_byagesexFemale_180724_EV.xlsx"))
pdataM_raw = readxl::read_xlsx(paste0(dirDataRaw, "/UN_pop_raw/EI_Popdta_byagesexMale_180724_EV.xlsx"))

# Country data
cdata = read.csv(paste0(dirDataRaw, "/EI_Countrydta_110325_EVK.csv"), sep=",")

# Surveillance implementation data
idata = readxl::read_xlsx(paste0(dirDataRaw,"/Implementation Questionnaire_110325_EV.xlsx"))      

# HAQI data
haqidata = read.csv(paste0(dirDataRaw, "/HAQI_raw/IHME_GBD_2019_HAQ_1990_2019_DATA_Y2022M012D21.csv"), sep=",")

# AMR data
adataAC = read.csv(paste0(dirDataRaw, "/EI_AMRdtaAC_110325_EV.csv"), sep=",")   # Country AMR data
adataDM = read.csv(paste0(dirDataRaw, "/EI_AMRdtaDM_110325_EV.csv"), sep=",")   # Country AMR data
adataNT = read.csv(paste0(dirDataRaw, "/EI_AMRdtaINT_110325_EV.csv"), sep=",")   # Country AMR data

# List of drug bug combinations
dbdata = read.csv(paste0(dirDataClean, "/updated_summary_dbc_longformat.csv")) # For the new reports, this list will have to be updated with all the new drug-bug combinations
...
creport = adataAC %>% group_by(Iso3, Year) %>%
  ...
  filter(Year == year) # This can be changed to the latest year of interest
...
# Export data
write.csv(cdata, paste0(dirDataClean, "/EI_Countrydta_AST_140325_EV.csv")) # Change file date to relevant date
...
# Export data
write.csv(pdataDM, paste0(dirDataClean, "/EI_PopdtaDM_140325_EV.csv"))
...
haqidata2019 <- haqidata %>% filter(year_id == 2019, indicator_name=="HAQ Index", haq_index_age_type=="Overall") # If newer HAQI data available, change year_id
...
# Export data
write.csv(haqidata2019, paste0(dirDataClean, "/EI_HAQIdta_Country_140325_EV.csv"))

#############################################################
# AMR data
#############################################################

# Drug-bug combinations for 2023 (using still those pathogen-drug-infection type combinations reported up until 2022)
######################################################################################################################
...
combinations2022$combined[present] # 1 not featuring; ""BLOOD-Staphylococcus aureus-Methicillin-resistance"
                                   # Makes sense, MRSA needs to be created.
...
# NOW WE NEED TO PUT BACK IN THE SINGLE 3GC ANTIBIOTICS
a1 <- adataAC %>% 
...
d8 = rbind(d7,a1)
check = d8%>% filter(Antibiotic %in%c("CTX", "CAZ", "CRO")& Pathogen=="ESCCOL"& Specimen=="BLOOD"|AntibioticName=="Third-generation cephalosporins" & Pathogen=="ESCCOL"& Specimen=="BLOOD") 
...
# Export data
write.csv(d10, paste0(dirDataClean, "/EI_AMRdtaAC_Pop_country_HAQI_140325_EV.csv"))

adataAC_cb = d9

write.csv(adataAC_cb, paste0(dirDataClean, "/final_linked_data/EI_AMRdtaAC_ANALYSES.csv"))
...
# NOW NEED TO PUT BACK IN THE SINGLE 3GC ANTIBIOTICS
a1 <- adataNT_c %>% 
  filter(Antibiotic %in%c("CTX", "CAZ", "CRO") & Pathogen=="ESCCOL" & Specimen=="BLOOD") %>%
  mutate(PercentResistant = ((coalesce(Resistant,0)/(coalesce(InterpretableAST,0))*100))
  )%>%
...
write.csv(d10, paste0(dirDataClean, "/EI_AMRdtaINT_Pop_Country_140325_EV.csv"))
...
# Export data
write.csv(adataDM, paste0(dirDataClean, "/EI_AMRdtaDM_Country_140325_EV.csv"))
...
# Take out all the lines which specify the origin of the sample; keep unknown age in and include them in model fitting
...
# Export data
write.csv(adataAS, paste0(dirDataClean, "/EI_AMRdtaINT_ANALYSES_noTESTING.csv")) 
...
idatao = read.csv(paste0(dirDataRaw,"/EI_Implementationdta_110325_EV.csv"), sep=",")      
...
idatac = left_join(idatac, pdata%>%filter(Year==year), by="Iso3")
...
# CHECK with curated data olga
sdatao = read.csv(paste0(dirDataRaw,"/EI_SurveillanceSites_110325_EV.csv"), sep=",")      
unique(sdatao$Iso3)
# 
...
# I AM GETTING COMPLETELY DIFFERENT NUMBERS SO LEAVE FOR NOW --> THAT IS BECAUSE THESE FROM
# OLGA HAVE BEEN MANUALLY UPDATED AFTER DATA VERIFICATION, USE THOSE
...
# Export data
write.csv(idatacss, paste0(dirDataClean, "/final_linked_data/EI_Implementationdta_Country_140325_EV.csv"))
write.csv(idatacs, paste0(dirDataClean, "/final_linked_data/EI_Implementation_usage_dta_140325_EV.csv"))
