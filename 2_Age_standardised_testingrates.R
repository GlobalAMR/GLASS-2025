##################################################################################################
# GLASS 2025 REPORT – AGE-STANDARDISED TESTING COVERAGE ANALYSIS
##################################################################################################

# Author: Esther van Kleef
# Date created: 30 July 2024
# Date last updated: 20 February 2026

# Purpose:
# This script evaluates different approaches for calculating age-standardised
# testing coverage (BCI per 1,000,000 population) for key GLASS drug–bug
# combinations. The objective is to identify a robust and stable method for
# standardising testing intensity across countries and regions before assessing
# the relationship between testing coverage and AMR prevalence.
#
# Key actions performed by this script:
# 1. Data preparation
#    - Load curated GLASS AMR datasets (country-level and age-sex stratified).
#    - Link population denominators and WHO world standard population.
#    - Define proxy antibiotic classes per syndrome and pathogen.
#
# 2. Age group restructuring
#    - Collapse 10-year age categories into broader groups (0–14, 15–64, 65+).
#    - Assess isolate sufficiency per age stratum (minimum thresholds).
#    - Evaluate proportion of missing age data per drug–bug combination.
#
# 3. Standardisation strategies compared
#    Age-standardised testing rates were calculated under multiple criteria:
#      (1) ≥10 isolates per age stratum + <15% missing age
#      (2) <15% missing age only
#      (3) ≥50 isolates (sensitivity analysis)
#      (4) Use all strata with ≥10 isolates (final selected approach)
#
#    Regional medians are used to impute testing coverage for countries
#    without sufficient age-stratified data.
#
# 4. Testing coverage vs AMR relationship
#    - Fit binomial GLMs (logit link) with quadratic terms for testing coverage.
#    - Examine residual patterns to identify empirical testing cut-offs.
#    - Define syndrome- and pathogen-specific testing thresholds.
#
# 5. Outputs
#    - Country- and region-level age-standardised testing rates.
#    - Comparative plots across standardisation criteria.
#    - Testing coverage vs AMR visualisations.
#    - Final analysis datasets for regression modelling.
#
# Final approach:
# Criteria (4) was selected as the primary method:
# include all age strata with ≥10 isolates, without excluding countries
# if some strata are sparse. This approach provides stable estimates while
# preserving maximum country inclusion and consistency with crude rates.
...
adataAS =  read.csv(paste0(dirDataClean, "/EI_AMRdtaINT_ANALYSES_NOTESTING.csv"), sep=",")   # Country AMR data
...
# Save as PDF
ggsave(file=paste0(dirOutput,"/Descriptives/Testing_vs_amrBLOOD.pdf"), 
                   plot = combined_plot, width = 12, height = 40)

ggsave(file=paste0(dirOutput,"/Descriptives/Testing_vs_amrUTI.pdf"), 
       plot = combined_plotUTI, width = 12, height = 30)

ggsave(file=paste0(dirOutput,"/Descriptives/Testing_vs_amrGI_GON.pdf"), 
       plot = combined_plotGI_GON, width = 12, height = 30)

# APPROACH FOR AGE-SEX STANDARDISATION
# -------------------------------------------------------------------------------------------
...
pdf(file=paste0(dirOutput,"/Descriptives/Proportion_with10isolatesormore_age_strata.pdf"))
print(p1)
dev.off()
...
pdf(file=paste0(dirOutput,"/Descriptives/Proportion_with10isolatesormore_age_stratabig.pdf"))
print(p2)
dev.off()
...
# }
#
# pdf(file=paste0(dirOutput,"/Descriptives/Age_distributions_testing_proxies.pdf"))
# for(i in 1:length(plots)){
#   print(plots[[i]])
# }
# dev.off()
...
#   theme(axis.text.x = element_text(angle = 90, hjust = 1))
#
# ggsave(file=paste0(dirOutput,"/Descriptives/st_testing_proxyclasses_all.pdf"),
#        plot = p0, width = 12, height = 10)
#
# ggsave(file=paste0(dirOutput,"/Descriptives/st_testing_crude_ecoli.pdf"),
#        plot = p1, width = 12, height = 10)
#
# ggsave(file=paste0(dirOutput,"/Descriptives/st_testing_crude_abacter.pdf"),
#        plot = p2, width = 12, height = 8)
#
# ggsave(file=paste0(dirOutput,"/Descriptives/st_testing_crude_kpn.pdf"),
#        plot = p3, width = 12, height = 10)
#
# ggsave(file=paste0(dirOutput,"/Descriptives/st_testing_crude_neiss.pdf"),
#        plot = p4, width = 12, height = 8)
#
# ggsave(file=paste0(dirOutput,"/Descriptives/st_testing_crude_shig.pdf"),
...
ggsave(file=paste0(dirOutput,"/Descriptives/CTA_not_enoughdata_agestand_testing_criteria1.pdf"), 
...
ggsave(file=paste0(dirOutput,"/Descriptives/CTA_not_enoughdata_agestand_testing_criteria2.pdf"), 
...
ggsave(file=paste0(dirOutput,"/Descriptives/Regional_age_st_testing_criteria_compared.pdf"), 
       plot = p2, width = 25, height = 15)
...
ggsave(file=paste0(dirOutput,"/Descriptives/CTA_not_enoughdata_agestand_testing_criteria3.pdf"), 
...
ggsave(file=paste0(dirOutput,"/Descriptives/CTA_not_enoughdata_agestand_testing_criteria4.pdf"), 
       plot = p, width = 25, height = 15)
...
# Of note, based on email Daniel Marcano Zamora (23 Sept 2024), Should take fluoroquinelones for shigella
unique(e1$combined)
...
# Save as PDF
ggsave(file=paste0(dirOutput,"/Descriptives/ST_Testing_vs_amrBLOOD.pdf"), 
       plot = combined_plot, width = 12, height = 40)

ggsave(file=paste0(dirOutput,"/Descriptives/ST_Testing_vs_amrUTI.pdf"), 
       plot = combined_plotUTI, width = 12, height = 30)

ggsave(file=paste0(dirOutput,"/Descriptives/ST_Testing_vs_amrGI_GON.pdf"), 
       plot = combined_plotGI_GON, width = 12, height = 30)
...
# These cut-offs were defined as per below lines 1620 - 1656 after visual inspection 
...
write.csv(cutoff, paste0(dirDataClean, "/final_linked_data/estimated_testing_cutoff.csv"))
...
pdf(file=paste0(dirOutput,"/Descriptives/Testing_coverage_cutoff2.pdf"),
    width=15, height=15)
print(panel_plot)
dev.off()
...
# NOW SAVE DATASETS FOR ANALYSES WITH THE STANDARDISED TESTING RATES
write.csv(f3, paste0(dirDataClean, "/final_linked_data/EI_AMRdtaINT_ANALYSES.csv"))
write.csv(h3, paste0(dirDataClean, "/final_linked_data/EI_AMRdtaAC_ANALYSES.csv"))
